...
func RegisterGatewayRoutes(
	...
	imageTaskCreateHandler := func(c *gin.Context) {
	...
	imageTaskGetHandler := func(c *gin.Context) {
		if getGroupPlatform(c) != service.PlatformOpenAI {
			c.JSON(http.StatusNotFound, gin.H{
				"error": gin.H{
					"type":    "not_found_error",
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/image-tasks", imageTaskCreateHandler)
		gateway.GET("/image-tasks/:id", imageTaskGetHandler)
	...
	r.POST("/image-tasks", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, imageTaskCreateHandler)
	r.GET("/image-tasks/:id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, imageTaskGetHandler)
